...
Prompts the user to open the .txt file created by the Roi-Group Manager plugin.
...
/*This prompts the user to open the .txt file created by the Roi-Group table plugin.  
 * Once the file is opened, this macro will read the number of groups then walk the user through analysis of each group.
*/

waitForUser("You are about to open the file containing group names.  Locate the .txt file under Seagate>Seagate.  Press 'OK' to proceed.");
groupNames= File.openDialog("Open Roi Groups File");
groupNamesString = File.openAsString(groupNames);


splitGroupNames = split(groupNamesString, ",");
//Array.print(splitGroupNames); //debug line
//print("length of splitGroupNames: " + lengthOf(splitGroupNames)); //debug line


numberGroups = lengthOf(splitGroupNames);
//print("numGroup value: ", numberGroups); //debug line


for (i=1; i<=numberGroups; i++){
	run("Roi Defaults...", "color=red stroke=0 group=i");
	waitForUser("ROI Group: " + splitGroupNames[i-1], "Leave dialog open until you finish tracing cell group.  Click 'OK' only when ready to move to next cell type");
}
